 ...
 public:
  ...
  static ServerConfig DefaultConfig() {
    return {
        ...
        .cert_chain_path = "ssl_creds/chain.pem",
        .private_key_path = "ssl_creds/privatekey.pem",
  ...
  static bool RunServers(const ServerConfig& config = ServerConfig{}) {
    ...
    // Start secure server only if TLS is not disabled and credentials are available
    if (!config.disable_tls && !config.secure_address.empty()) {
      auto creds = CreateSecureServerCredentials(config.private_key_path, config.cert_chain_path);
      if (creds) {
    ...
    // Start plaintext server if not disabled
    if (!config.disable_plaintext && !plaintext_server_) {
